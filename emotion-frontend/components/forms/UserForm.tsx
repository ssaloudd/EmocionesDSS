...
const UserForm: React.FC<UserFormProps> = ({ type, data, onSubmit, onClose }) => {
  ...
  const [rol, setRol] = useState<'alumno' | 'docente' | 'admin'>(data?.rol || 'alumno'); // Valor por defecto 'alumno'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!username || !firstName || !lastName || !email || !genero || !ci || !rol) {
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 flex flex-col gap-4">
      <h2 className="text-lg font-semibold text-center">
        {type === "create" ? "Crear Nuevo Usuario" : `Actualizar Usuario: ${data?.first_name} ${data?.last_name}`}
      </h2>
      ...
      <div>
        ...
        <select id="rol" value={rol} onChange={(e) => setRol(e.target.value as 'alumno' | 'docente' | 'admin')}
          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" required>
          <option value="">Seleccione</option>
      ...
      {type === 'create' || (type === 'update' && password) ? ( // Muestra el campo de contraseña solo si es create o si se está editando la contraseña
        <div>
          <label htmlFor="password" className="block text-sm font-medium text-gray-700">Contraseña {type === 'update' ? '(dejar en blanco para no cambiar)' : ''}:</label>
          <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)}
            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
            required={type === 'create'} />
        </div>
      ) : (
        <button type="button" onClick={() => setPassword('temp')} className="text-blue-500 hover:underline text-sm self-start">Cambiar contraseña</button>
      )}

      <button type="submit" className="bg-lamaSky text-white py-2 px-4 rounded-md border-none w-max self-center mt-4">
